...
class HealthKitManager: ObservableObject {
    ...
    @Published var monthlyData: [DailyHealthSummary] = []
    ...
    // MARK: - Fetch Monthly Data

    func fetchMonthlyData(for month: Date = .now) async {
        ...
        while currentDate < monthEnd && currentDate <= today {
            ...
            // Reuse existing weeklyData for recent days to avoid redundant HealthKit queries
            if let cached = weeklyData.first(where: { calendar.isDate($0.date, inSameDayAs: startOfDay) }) {
                dailySummaries.append(cached)
            } else {
                var metrics: [HealthMetricType: Double] = [:]
                async let steps = Self.fetchSumOffMain(healthStore: healthStore, type: .stepCount, unit: .count(), start: startOfDay, end: endOfDay)
                async let hr = Self.fetchHeartRateOffMain(healthStore: healthStore, start: startOfDay, end: endOfDay)
