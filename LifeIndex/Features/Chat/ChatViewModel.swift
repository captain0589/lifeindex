...
class ChatViewModel: ObservableObject {
    ...
    @Published var enhancedContext: EnhancedHealthContext?

    private let persistence = ChatPersistence.shared
    private let contextBuilder = HealthContextBuilder.shared
    ...
    /// Update with enhanced context including trends and user profile
    func updateEnhancedContext(
    ...
    private func buildHealthContextString() -> String {
        // Use enhanced context if available (includes trends, user profile, etc.)
        if let enhanced = enhancedContext {
            return enhanced.toPromptContext()
        }
