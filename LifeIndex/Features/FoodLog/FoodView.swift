...
struct FoodView: View {
    ...
    private var burnedCalories: Double {
        // Get burned calories for the selected date, not just today
        if Calendar.current.isDateInToday(selectedDate) {
            return healthKitManager.todaySummary.value(for: .activeCalories) ?? 0
        } else {
            // Look up in weekly data for past days
            let dayData = healthKitManager.weeklyData.first {
                Calendar.current.isDate($0.date, inSameDayAs: selectedDate)
            }
            return dayData?.metrics[.activeCalories] ?? 0
        }
    }
    ...
    // Generate dates for multiple weeks (4 weeks back + current week)
    private var multiWeekDates: [Date] {
        let calendar = Calendar.current
        let today = calendar.startOfDay(for: Date())
        let weekday = calendar.component(.weekday, from: today)
        let startOfCurrentWeek = calendar.date(byAdding: .day, value: -(weekday - 1), to: today)!
        // Go back 3 weeks from start of current week
        let startDate = calendar.date(byAdding: .day, value: -21, to: startOfCurrentWeek)!
        // Generate 28 days (4 weeks)
        return (0..<28).compactMap { calendar.date(byAdding: .day, value: $0, to: startDate) }
    }
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: Theme.Spacing.lg) {
                    // MARK: - Scrollable Week Day Selector
                    ScrollableWeekSelector(
                        dates: multiWeekDates,
                        selectedDate: $selectedDate,
                        accentColor: Theme.calories,
...
// MARK: - Scrollable Week Selector

private struct ScrollableWeekSelector: View {
    let dates: [Date]
    ...
    var body: some View {
        VStack(spacing: Theme.Spacing.xs) {
            // Week label showing month range
            if let firstDate = dates.first(where: { Calendar.current.isDate($0, inSameDayAs: selectedDate) }) ?? dates.last {
                let weekStart = weekStartFor(date: firstDate)
                ...
                HStack {
                    Text(weekRangeString(start: weekStart, end: weekEnd))
                        ...
                        .foregroundStyle(Theme.secondaryText)
                    Spacer()
                    Text("calendar.swipeHint".localized)
            ...
            ScrollViewReader { proxy in
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: Theme.Spacing.lg) {
                        ForEach(weekGroups, id: \.first) { week in
                            WeekRow(
                                dates: week,
                                selectedDate: $selectedDate,
                ...
                .onAppear {
                    ...
                    if let todayWeek = weekGroups.first(where: { week in
                        week.contains { Calendar.current.isDateInToday($0) }
                    }), let firstDate = todayWeek.first {
    ...
    private func weekStartFor(date: Date) -> Date {
        let calendar = Calendar.current
    ...
    private var expandedContent: some View {
        VStack(spacing: Theme.Spacing.md) {
            ...
            Button {
                ...
                let btnLabel = "\("food.useRecommended".localized) (\(calculatedGoal) \("units.kcal".localized))"
                Text(btnLabel)
                    .font(.system(.subheadline, design: .rounded, weight: .medium))
                    .foregroundStyle(Theme.calories)
                    .frame(maxWidth: .infinity)
