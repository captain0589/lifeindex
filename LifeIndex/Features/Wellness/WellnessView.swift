...
struct WellnessView: View {
    ...
    @State private var dateExplicitlySelected = false
            ...
            ScrollView {
                VStack(spacing: Theme.Spacing.lg) {
                    ...
                    MoodCalendarCard(
                        ...
                        onDateTap: { date in
                            // Single tap selects the date and shows entries
                            selectedDate = date
                            dateExplicitlySelected = true
                            loadLogsForSelectedDate()
                        },
                        onDoubleTap: { date in
                            // Double tap opens the mood logger
                            selectedDate = date
                            loggerDate = date
                            loadLogsForSelectedDate()
                            editingLog = nil
                    ...
                    if dateExplicitlySelected {
                        if !selectedDayLogs.isEmpty {
                            ...
                            NoJournalDataView(date: selectedDate)
                            .padding(.horizontal)
                        }
                    }
...
// MARK: - No Journal Data Empty State

private struct NoJournalDataView: View {
    let date: Date
        ...
        VStack(spacing: Theme.Spacing.sm) {
            ...
            VStack(spacing: Theme.Spacing.md) {
                Image(systemName: "book.closed")
                    .font(.system(size: 32))
                    .foregroundStyle(Theme.mood.opacity(0.5))
