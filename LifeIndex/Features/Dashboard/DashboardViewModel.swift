...
class DashboardViewModel: ObservableObject {
    ...
    // Monthly scores for calendar
    @Published var monthlyScores: [(date: Date, score: Int)] = []
    @Published var monthlyDetailData: [DailyHealthSummary] = []
    ...
    var dateFilterLabel: String {
        ...
        let date = selectedCalendarDate.flatMap { greg.isDateInToday($0) ? nil : $0 } ?? Date()
    ...
    /// Exposed for MainTabView to pass to HealthAIChatView
    func buildExposedHealthContext() -> HealthContext {
        ...
        return HealthContext(
            ...
            workoutMinutes: workoutMinutesValue > 0 ? workoutMinutesValue : nil,
            insights: insights.map { $0.text },
    ...
    func loadData(forceRefresh: Bool = false) async {
        ...
        // Monthly scores for calendar (fetch after weekly so cache hits)
        await manager.fetchMonthlyData(for: displayedMonth)
        buildMonthlyScores(from: manager.monthlyData)
    ...
    // MARK: - Load Monthly Data (for month navigation in calendar)

    func loadMonthlyData(for month: Date) async {
        guard let manager = healthKitManager else { return }
        displayedMonth = month
        await manager.fetchMonthlyData(for: month)
        buildMonthlyScores(from: manager.monthlyData)
    }

    private func buildMonthlyScores(from data: [DailyHealthSummary]) {
        let calendar = Calendar(identifier: .gregorian)
        self.monthlyDetailData = data
        monthlyScores = data.compactMap { day in
            guard !day.metrics.isEmpty else { return nil }
            let isToday = calendar.isDateInToday(day.date)
            let score: Int
            if isToday && !showingYesterdayData {
