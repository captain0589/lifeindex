...
struct LifeIndexSection: View {
    ...
    var onTapSteps: (() -> Void)? = nil
    var onTapSleep: (() -> Void)? = nil
    var onTapHeart: (() -> Void)? = nil
    ...
    var body: some View {
        VStack(spacing: Theme.Spacing.lg) {
            ...
            VStack(spacing: Theme.Spacing.lg) {
                ...
                // MARK: - Quick Summary Stats (Steps, Sleep, Heart Rate)
                if steps > 0 || sleepMinutes != nil || heartRate != nil {
                    HStack(spacing: Theme.Spacing.sm) {
                        if steps > 0 {
                            QuickStatCard(
                                icon: "figure.walk",
                                value: "\(Int(steps).formatted())",
                                label: "metrics.steps".localized,
                                color: Theme.steps,
                                action: onTapSteps
                            )
                        }
                        ...
                        if let hr = heartRate {
                            QuickStatCard(
                                icon: "heart.fill",
                                value: "\(Int(hr))",
                                label: "metrics.heartRate".localized,
                ...
                // Integrated Insights Section (below summary stats)
                if isLoadingInsights || !insights.isEmpty || aiShortSummary != nil {
                    VStack(alignment: .leading, spacing: Theme.Spacing.sm) {
                        ...
                        if isLoadingInsights && insights.isEmpty && aiShortSummary == nil {
                            HStack(spacing: Theme.Spacing.sm) {
                                ProgressView()
                                    .controlSize(.small)
                                Text("insights.loading".localized)
                        ...
                        // Individual insights first (text lines)
                        ...
                        // AI Summary with expand/collapse (purple box)
...
// MARK: - Quick Stat Card (for Steps, Sleep, Heart Rate)
