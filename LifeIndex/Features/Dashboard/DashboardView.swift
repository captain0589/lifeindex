...
struct DashboardView: View {
    ...
    @State private var syncedTimeString = ""
    @State private var scrollToHeartSection = false
    ...
    var body: some View {
        NavigationStack {
            ...
            ScrollView {
                VStack(spacing: Theme.Spacing.xl) {
                    ...
                    VStack(spacing: Theme.Spacing.sm) {
                            ...
                            HStack(spacing: Theme.Spacing.md) {
                                ...
                                Button {
                                    Task {
                                        ...
                                        syncedTimeString = fmt.string(from: Date())
                                ...
                                // Settings button — extra leading gap
                                Button {
                                    showSettings = true
                                } label: {
                        ...
                        // Row 2: Date filter button
                        HStack {
                            Button {
                                showCalendar = true
                            } label: {
                                HStack(spacing: 6) {
                                    Text(viewModel.dateFilterLabel)
                    ...
                    if viewModel.hasData {
                        ...
                        // MARK: - Combined LifeIndex Section (Score + Summary + Insights)
                        LifeIndexSection(
                            ...
                            isLoadingInsights: viewModel.isLoading,
                            onRequestDetailed: {
                                Task { await viewModel.generateDetailedSummary() }
                            },
                            onTapSteps: {
                                selectedTab = 2 // Fitness tab
                            },
                            onTapSleep: {
                                selectedTab = 1 // Sleep tab
                            },
                            onTapHeart: {
                                withAnimation {
                                    scrollProxy.scrollTo("heartSection", anchor: .top)
                                }
                            }
                        )
            ...
            .sheet(isPresented: $showCalendar) {
                MonthlyCalendarSheet(viewModel: viewModel)
        ...
        .overlay(alignment: .top) {
            if showRefreshSuccess {
                HStack(spacing: 8) {
                    ...
                    Text(String(format: "dashboard.synced".localized, syncedTimeString))
                        .font(.system(size: 13, weight: .semibold, design: .rounded))
                        .foregroundStyle(.white)
                }
                .padding(.horizontal, 16)
                .padding(.vertical, 10)
                .background(Color(red: 0.18, green: 0.62, blue: 0.38), in: Capsule())
                .shadow(color: Color(red: 0.18, green: 0.62, blue: 0.38).opacity(0.35), radius: 10, x: 0, y: 4)
                .padding(.top, 8)
...
struct ComparisonTile: View {
    ...
    let value: String
    let color: Color
    var labelColor: Color = Theme.primaryText
    var tileBackground: AnyShapeStyle = AnyShapeStyle(Theme.cardBackground)
