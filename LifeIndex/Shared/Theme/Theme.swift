...
enum Theme {
    ...
    /// Adaptive card surface: white in light mode, secondarySystemBackground in dark mode.
    /// Use this instead of .regularMaterial or Theme.secondaryBackground for content cards.
    ...
    // Header color — plain adaptive background (no gradient)
    static let headerColor = Color(.systemBackground)

    // Dynamic header color based on color scheme (plain, no gradient)
...
struct CardStyle: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            ...
            .shadow(
                color: colorScheme == .dark ? Color.black.opacity(0.18) : Color.black.opacity(0.08),
...
struct PageBackground: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .background {
                // Light mode: grouped (light gray) so cards stand out
                // Dark mode: system background (near-black)
...
// MARK: - Card Background Modifier (no padding, just background + shadow)
